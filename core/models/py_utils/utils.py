...
def _topk(scores, K=20, min_score=0):
    ...
    if not min_score==0:
        keep_inds  = (topk_scores[0] > min_score)
...
#decode the result to detections
def _decode(
    ...
    #inds表示在w*h里面的位置
#     tl_scores, tl_inds, tl_clses, tl_ys, tl_xs = _topk(tl_heat, K=K,min_score=0.15)
#     
#     K = tl_scores.shape[1]
# #     print("K is {}".format(K))
#     if K<10:
...
#         tl_scores, tl_inds, tl_clses, tl_ys, tl_xs = _topk(tl_heat, K=K)
    ...
    #k个左上角点和右下角点两两配对，总共有k*k个配对，即k*k个box
    ...
    #返回top num_dets个值和对应得下标
    t_n = num_dets
    if scores.shape[1]<num_dets:
        t_n = K
        
    scores, inds = torch.topk(scores, t_n)
...
class convolution(nn.Module):
    def __init__(self, k, inp_dim, out_dim, stride=1, with_bn=True,with_relu=True):
        ...
        self.relu = nn.ReLU(inplace=True) if with_relu else nn.Sigmoid()
