...
'''
1.定义网络的各个模块
2.损失函数=CornerNet_Loss
hg_net>hg>hg_modules
'''
class model(hg_net):
    ...
    def _sub_merge_mod(self):
        return nn.Sequential(
            nn.Conv2d(256, 256, (1, 1), stride=2,bias=False),
            nn.BatchNorm2d(256)
        )

    def __init__(self):
        stacks  = 2
        #先对原图进行3次下采样，变成64*64，是得inference time从111ms->41ms
        pre = nn.Sequential(
        ...
        hg_mods = nn.ModuleList([
            hg_module(
                #n 4: 表示下采样几次：128>64>32>16
                #dims: 每层的通道数
                #modules: fire_module 模块数
                4, [256, 256, 384, 384, 512], [2, 2, 2, 2, 4],
                make_pool_layer=make_pool_layer, #none
                make_unpool_layer=make_unpool_layer, #stride=2,up*2
                make_up_layer=make_layer, #keep
                make_low_layer=make_layer, #keep
                make_hg_layer_revr=make_layer_revr, #keep
                make_hg_layer=make_hg_layer #down/2
        ...
        #added by su
        subs = nn.ModuleList([self._sub_merge_mod() for _ in range(stacks)])
        merges = nn.ModuleList([self._merge_mod() for _ in range(stacks)])
        cnv_d2s = nn.ModuleList([convolution(3, 256, 256, stride=1) for _ in range(stacks)])
        
        
        hgs = hg(pre, hg_mods, cnvs, inters, cnvs_, inters_,subs,merges,cnv_d2s) 
        
        
        #构造各个分支
        tl_modules = nn.ModuleList([corner_pool(256, TopPool, LeftPool) for _ in range(stacks *2)])
        br_modules = nn.ModuleList([corner_pool(256, BottomPool, RightPool) for _ in range(stacks *2)])
        
        #80:num of classes
        #output: w*h*80
        cls_num = 20
        tl_heats = nn.ModuleList([self._pred_mod(cls_num) for _ in range(stacks *2)])
        br_heats = nn.ModuleList([self._pred_mod(cls_num) for _ in range(stacks *2)])
        ...
        #output:w*h*1
        tl_tags = nn.ModuleList([self._pred_mod(1) for _ in range(stacks *2)])
        br_tags = nn.ModuleList([self._pred_mod(1) for _ in range(stacks *2)])
        
        #output:w*h*1
        tl_offs = nn.ModuleList([self._pred_mod(2) for _ in range(stacks *2)])
        br_offs = nn.ModuleList([self._pred_mod(2) for _ in range(stacks *2)])
